...
static void update() {
    if (!ticks) {
        ...
        p->vy = 1;
        p->isAlive = true;
        pointIndex = cgl_wrap(pointIndex + 1, 0, CROSSLINE_MAX_POINT_COUNT);
        ...
        l->addedCount = 2;
        l->isAlive = true;
        lineIndex = cgl_wrap(lineIndex + 1, 0, CROSSLINE_MAX_LINE_COUNT);
    ...
    float scr = sqrtf(difficulty) * 0.03f;
    if (pointMaxY < 40) {
        scr += (40 - pointMaxY) * 0.1f;
    }
