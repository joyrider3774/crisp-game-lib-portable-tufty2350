...
static void addCoinPenalty(float x, bool isShotHit) {
    ...
    addScore(-coinPenaltyMultiplier, x + sqrtf(coinPenaltyMultiplier) * 4, 50);
...
static void update() {
    ...
    const float vx = (-difficulty - penaltyVx) * 0.5f;
    color = RED;
    for (int i = 0; i < ceilf(penaltyVx * 2 + 0.1f); i++) {
        text("<", i * 6 + 3, 48);
    }
    color = BLACK;
    nextObjDist += vx;
    ...
    if (nextObjDist < 0) {
        char type = prevType != 'c' && rnd(0, 1) < 0.5f ? 'c' : 'e';
        prevType = type;
        int c = rndi(3, 9);
        float x = 200;
        if (type == 'c') {
            ...
            for (int i = 0; i < c; i++) {
                float size = rnd(0, 1) < 0.3f ? rndi(2, 6) : 1;
                FOR_EACH(objs, j) {
                    ASSIGN_ARRAY_ITEM(objs, j, Obj, o);
                    if (!o->isAlive) {
                        o->x = x;
   ...
   if (character("a", 10, 30).isColliding.character['b']) {
    ...
    FOR_EACH(inhalingCoins, i) {
        ...
        if (col.isColliding.character['d'] || col.isColliding.character['e']) {
            ...
            play(COIN);
            addScore(coinMultiplier, 10 + sqrtf(coinMultiplier) * 4, 30);
