...
static void drawHex(Vector *cp, float s) {
  ...
  addWithAngle(&p1, (5.5f * M_PI) / 3, s);
  for (int i = 0; i < HEXMIN_LANE_COUNT; i++) {
    vectorSet(&p2, p1.x, p1.y);
    addWithAngle(&p1, ((i + 0.5f) * M_PI) / 3, s);
...
static void update() {
  ...
  if (input.isJustPressed || (keyRepeatTicks < 0 && input.isPressed)) {
    keyRepeatTicks = 15 / sqrtf(difficulty);
    play(LASER);
    int oa = input.left.isPressed ? -1 : 1;
    arrowAngle = cgl_wrap(arrowAngle + oa, 0, 6);
    multiplier = ceilf(multiplier * 0.9f) - 1;
    if (multiplier < 1) {
      multiplier = 1;
    }
  }
  ...
  Vector p;
  for (int i = 0; i < HEXMIN_LANE_COUNT; i++) {
    int oa = cgl_wrap(i - arrowAngle, 0, 6);
    vectorSet(&p, 50, 50);
  ...
  if (laneChangeTicks < 0) {
    ...
    laneChangeTicks += 30 / sqrtf(difficulty);
  }
  for (int j = 0; j < HEXMIN_LANE_COUNT; j++) {
    ...
    if (lanes[j] == 4) {
      int oa = cgl_wrap(j - arrowAngle, 0, 6);
      if (oa == 0 || oa == 2) {
        play(COIN);
