...
static void update() {
    ...
    player.speed += ((input.isPressed ? 9.0f : 1.0f) * sqrtf(difficulty) - player.speed) * 0.2f;
    player.va += (player.tva - player.va) * 0.2f;
    if (player.y > 80.0f) {
        player.y -= 0.01f;
    }
    player.angle += player.va * sqrtf(difficulty) * 0.05f;
    ...
    if (nextWallDist < 0) {
        ...
        w->isBonus = isBonus;
        w->isAlive = true;
        wallIndex = cgl_wrap(wallIndex + 1, 0, CTOWER_MAX_WALL_COUNT);
    ...
    FOR_EACH(walls, i) {
        ...
        float fa = clamp(cgl_wrap(wa - w->width / 2.0f, 0.0f, M_PI * 2.0f), M_PI / 2.0f, (M_PI / 2.0f) * 3.0f);
        float ta = clamp(cgl_wrap(wa + w->width / 2.0f, 0.0f, M_PI * 2.0f), M_PI / 2.0f, (M_PI / 2.0f) * 3.0f);
    ...
    float ho = fmodf(player.height, 400.0f) - 100.0f;
    // Draw backgrounds
    color = CYAN;
    ...
    FOR_EACH(walls, i) {
        ...
        if (w->isFloating) {
            float wa = player.angle - w->angle;
            float fa = clamp(cgl_wrap(wa - w->width / 2.0f, -M_PI, M_PI), -M_PI / 2.0f, M_PI / 2.0f);
            float ta = clamp(cgl_wrap(wa + w->width / 2.0f, -M_PI, M_PI), -M_PI / 2.0f, M_PI / 2.0f);
