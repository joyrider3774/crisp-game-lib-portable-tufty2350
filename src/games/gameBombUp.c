...
static void update() {
    ...
    // Update rocks
    nextRockTicks--;
    nextRockVelX += rnd(-0.1f, 0.1f) * sqrtf(difficulty); // Simplified rnds(0.1)
    nextRockVelX *= 0.99f;
    ...
    if (nextRockTicks < 0) {
        ...
        float yvel = isBottom ? -rnd(0.5f, sqrtf(difficulty)) : player.vel.y / 2;
        vectorSet(&r->vel, 0, yvel);
        r->isAlive = true;
        rockIndex = cgl_wrap(rockIndex + 1, 0, MAX_ROCKS);
