...
static void addPanel() {
  ...
  panelIndex = cgl_wrap(panelIndex + 1, 0, LADDER_DROP_MAX_PANEL_COUNT);
  Vector size;
...
static void update() {
  ...
  FOR_EACH(panels, i) {
    ...
    SKIP_IS_NOT_ALIVE(p);
    if (p->state == 0) {
      p->pos.x += pvx * sqrtf(difficulty) * 1.5f;
      nextPanelX = p->pos.x;
      if (p->pos.x < -9) {
        pvx *= -1;
        p->pos.x = -9;
      ...
      if (cl['e' - 'a'] || cl['f' - 'a']) {
        ...
        if (p->hasCoin) {
          ...
          TIMES(p->size.x, x) {
            ...
            c->isAlive = true;
            coinIndex = cgl_wrap(coinIndex + 1, 0, LADDER_DROP_MAX_COIN_COUNT);
          }
        }
        addPanel();
      }
  ...
  if (player.state == 0 || player.state == 3) {
    player.pos.x += player.vx * sqrtf(difficulty) * 0.4f;
    ...
    play(HIT);
    player.pos.y -= sqrtf(difficulty) * 0.3f;
    color = TRANSPARENT;
    ...
    if (!c['e'] && c['f']) {
      ...
      player.pos.x += player.vx * sqrtf(difficulty) * 0.5f;
      player.state = 0;
    }
    color = BLACK;
    ...
    play(HIT);
    player.pos.y += sqrtf(difficulty) * 0.4f;
    color = TRANSPARENT;
    bool *c =
