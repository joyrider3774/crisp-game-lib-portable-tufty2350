...
static void update() {
    ...
    if (!bar(VEC_XY(rope.from), rope.length, rope.angle).isColliding.rect[LIGHT_BLACK]) {
        rope.length += sqrtf(difficulty) * 2;
        } else {
        rope.length = 0;
        }
    }
    ...
    if (ballsAlive == 0) {  // No active balls
        ...
        if (nextBallTicks > 60) {
            ...
            for (int i = 0; i < nextBallCount; i++) {
                ...
                b->invincibleTicks = 0;
                b->isAlive = true;
                ballIndex = cgl_wrap(ballIndex + 1, 0, MAX_BALL_COUNT);
            }
        }
    }
    ...
    FOR_EACH(balls, i) {
        ...
        if (arc(b->pos.x, b->pos.y, b->radius, 0, M_PI * 2).isColliding.rect[BLACK] && 
            ...
            play(COIN);
            float s = sqrtf(b->vel.x * b->vel.x + b->vel.y * b->vel.y);
            float invincibleTicks = 30 / sqrtf(difficulty);
            float oa = cgl_wrap(vectorAngle(&b->vel) - rope.angle - M_PI, -M_PI, M_PI);
    ...
    if (nearestBall != NULL) {
        ...
        vectorMul(&delta, 0.1f * sqrtf(difficulty));
    ...
    if (input.isPressed) {
        ...
        player.angle = cgl_wrap(player.angle, 0, 4);
    }
