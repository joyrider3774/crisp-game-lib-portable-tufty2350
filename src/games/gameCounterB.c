...
static void update() {
    if (!ticks) {
        ...
        e->beamLength = 0;
        e->isAlive = true;
        enemyIndex = cgl_wrap(enemyIndex + 1, 0, COUNTER_B_MAX_ENEMY_COUNT);
    ...
    float scr = (player.pos.x - player.baseX) * 0.1f;
    player.baseX += (30 - player.baseX) * 0.001f;
    ...
    color = YELLOW;
    rect(0, 90, 200, 10);
    scrX = cgl_wrap(scrX - scr, 0, 200);
    color = WHITE;
    rect(scrX, 90, 2, 10);

    float beamSpeed = 4 * sqrtf(difficulty);
    player.speed += ((input.isPressed ? 0.2f : 2) * sqrtf(sqrtf(difficulty)) - player.speed) * 0.2f;
    ...
    if (counter.hasPos) {
        counter.pos.x -= scr;
        color = PURPLE;
        arc(VEC_XY(counter.pos), counter.radius, ticks * 0.1f, ticks * 0.1f + M_PI * 2);
        particle(VEC_XY(counter.pos), 1, counter.radius * 0.1f, 0, M_PI * 2);
        
        if (counter.enemy == NULL) {
            counter.radius -= sqrtf(difficulty) * 2;
            if (counter.radius < 1) {
                counter.hasPos = false;
            }
        } else {
            counter.radius = clamp(
                counter.radius + (input.isPressed ? 1 : -2) * sqrtf(difficulty),
                0,
                30
            );
            if (counter.radius < 1) {
    ...
    if (nextEnemyTicks < 0) {
        ...
        e->speed = rnd(1, sqrtf(difficulty));
        e->beamLength = -rnd(0, 1) * sqrtf(difficulty) * 20;
        e->isAlive = true;
        enemyIndex = cgl_wrap(enemyIndex + 1, 0, COUNTER_B_MAX_ENEMY_COUNT);
    ...
    FOR_EACH(enemies, i) {
        ...
        if (counter.enemy == e && counter.hasPos) {
            ...
            e->beamLength += beamSpeed * 0.2f;
            if (e->beamLength - beamSpeed * 0.3f < 0 && e->beamLength >= 0) {
                play(LASER);
                color = RED;
                particle(VEC_XY(e->pos), 9, 3, e->angle, 0.5f);
            }
        }
        ...
        if (e->beamLength < -999) {
            if (e->speed < 1) {
                e->speed += (1 - e->speed) * 0.1f;
            }
            if (e->angle > 0) {
                e->angle += (M_PI - e->angle) * 0.05f;
            }
        }
