...
static void update() {
    ...
    if (barrelAddingTicks < 0) {
        ...
        b->angle = rnd(0, 99);
        b->isAlive = true;
        barrelIndex = cgl_wrap(barrelIndex + 1, 0, MAX_BARREL_COUNT);
    ...
    if (mode == RUN) {
        ...
        pos.y += vel.y;
        vel.y += input.isPressed ? 0.1f : 0.2f;
        if (pos.y > 85) {
            pos.y = 86;
            if (input.isPressed) {
                play(JUMP);
    ...
    anim += df * (input.isPressed ? 0.1f : 0.2f) * (mode == RUN ? 1 : 0.5f);
