...
static void update() {
    ...
    pillar.angleVel *= 1 - 0.1f * sqrtf(difficulty);
    ...
    if (nextCoinTicks < 0) {
        ...
        coin->isAlive = true;
        coinIndex = cgl_wrap(coinIndex + 1, 0, MAX_COINS);
        
        coinX = cgl_wrap(coinX + rnd(-1.0f, 1.0f) + scr, -20, 120);
        nextCoinTicks = 5;
    }
    ...
    FOR_EACH(coins, i) {
        ...
        if (cl.isColliding.rect[BLACK] || cl.isColliding.rect[RED] || 
            ...
            for (int t = 0; t < times; t++) {
                addScore(ceilf(pillar.length), coin->pos.x, coin->pos.y - t * 6);
                if (pillar.length < 64) {
                    pillar.length++;
                }
            }
