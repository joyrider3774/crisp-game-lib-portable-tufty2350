...
static void update() {
  ...
  if (input.isPressed) {
    ufo.power += (1 - ufo.power) * 0.05f;
  } else {
    ufo.power *= 0.9f;
  }
  ufo.pos.x += ufo.vx * sqrtf(difficulty) * (input.isPressed ? 0.5f : 1);
  if (!input.isPressed) {
    ufo.angle = clamp(ufo.angle - ufo.vx * sqrtf(difficulty) * 0.07f, -M_PI / 4,
                      M_PI / 4);
  }
  color = BLACK;
  ...
  if (nextTankTicks < 0) {
    ...
    t->vx = vx;
    t->angle = -M_PI / 2;
    t->angleVel = av * 0.002f;
    t->speed = 0.1f / sqrtf(av) * sqrtf(difficulty);
    t->fireTicks = t->fireInterval = fireInterval;
    t->fireSpeed = rnd(1, 1.5f) * sqrtf(difficulty);
    t->isAlive = true;
    nextTankTicks = rnd(60, 80) / sqrtf(difficulty);
    tankIndex = cgl_wrap(tankIndex + 1, 0, REFLECTOR_MAX_TANK_COUNT);
  }
  color = LIGHT_RED;
  ...
  FOR_EACH(explosions, i) {
    ...
    SKIP_IS_NOT_ALIVE(e);
    e->ticks--;
    float r = e->radius * sinf(e->ticks / e->duration * M_PI);
  ...
  FOR_EACH(tanks, i) {
    ...
    if (t->fireTicks < 0) {
      ...
      b->isAlive = true;
      bulletIndex = cgl_wrap(bulletIndex + 1, 0, REFLECTOR_MAX_BULLET_COUNT);
  ...
  FOR_EACH(bullets, i) {
    ...
    if (b->vel.y > 0 &&
      ...
      play(EXPLOSION);
      float s = vectorLength(&b->vel) / sqrtf(difficulty);
      float radius = s * s;
      float duration = sqrtf(radius) * 9;
      ...
      e->isAlive = true;
      explosionIndex =
          cgl_wrap(explosionIndex + 1, 0, REFLECTOR_MAX_EXPLOSION_COUNT);
      b->isAlive = false;
      continue;
    }
    b->isAlive =
