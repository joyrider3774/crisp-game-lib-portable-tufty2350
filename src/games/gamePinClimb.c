...
static void addPin(float x, float y) {
  ...
  p->pos.y = y;
  p->isAlive = true;
  pinIndex = cgl_wrap(pinIndex + 1, 0, PIN_CLIMB_MAX_PIN_COUNT);
}
...
static void update() {
  ...
  if (input.isPressed) {
    ...
    cord.length += (cordLength - cord.length) * 0.1f;
  }
  cord.angle += difficulty * 0.05f;
  ...
  if (nextPin != NULL) {
    play(POWER_UP);
    addScore(ceilf(distanceTo(&cord.pin->pos, VEC_XY(nextPin->pos))),
             VEC_XY(nextPin->pos));
    cord.pin = nextPin;
    cord.length = cordLength;
  }
