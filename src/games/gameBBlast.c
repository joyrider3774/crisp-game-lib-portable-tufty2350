...
static void update() {
    ...
    FOR_EACH(balls, i) {
        ...
        b->radius += (b->targetRadius - b->radius) * 0.05f;
        ...
        // Physics
        b->vel.y += 0.05f;
        vectorMul(&b->vel, 0.95f);
        float velLength = sqrtf(b->vel.x * b->vel.x + b->vel.y * b->vel.y);
        if (velLength > 1) {
            vectorMul(&b->vel, 0.5f);
        }
    ...
    if (radius >= 0 && input.isPressed) {
        radius += 0.5f * difficulty * (1 - radiusSlowTicks / 60);
        color = YELLOW;
        thickness = 2;
        ...
        FOR_EACH(balls, i) {
            ...
            if (distanceTo(&b->pos, pos.x, pos.y) - b->radius - radius < 0) {
                if (b->isRed) {
                    ...
                    play(COIN);
                    color = BLACK;
                    particle(b->pos.x, b->pos.y, b->radius, b->radius * 0.3f, M_PI * 2, 0.4f);
                }
