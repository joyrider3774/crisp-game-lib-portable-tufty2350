...
static void update() {
    ...
    if (!ticks) {
        ...
        passedCount = 0;
        nextBarDist = 50;
        nextBarIndex = cgl_wrap(nextBarIndex + 1, 0, MAX_BARS);
    }
    
    // Screen scrolling logic
    bool ip = input.isPressed;
    float scr = difficulty * 0.05f;
    if (ball.pos.x > 30) {
        scr += (ball.pos.x - 30) * 0.1f;
    }
    ball.pos.x -= scr;
    ...
    if (ball.bar != NULL) {
        // Update bar position
        ball.barPos += (ip ? -1 : 1) * difficulty * 0.2f;
        ...
        if (fabsf(ball.barPos) > ball.bar->width * 0.65f) {
            ball.bar = NULL;
            vectorSet(&ball.vel, 0, 0);
        } else if (ball.barPos > 0) {
            // Prediction visualization
            color = ball.barPos > ball.bar->width * 0.45f ? LIGHT_RED : LIGHT_BLACK;
    ...
    FOR_EACH(bars, i) {
        ...
        if (barCollision.isColliding.rect[BLACK] && ball.bar == NULL) {
            ...
            ball.grv = 0.1f * difficulty;
            b->isPassed = true;
        }
        
        // Passing bars
        if (!b->isPassed && b->pos.x + b->width * 0.5f < ball.pos.x) {
            play(COIN);
            b->isPassed = true;
            passedCount++;
