...
static void update() {
    ...
    float scr = bird.pos.x > 30 ? (bird.pos.x - 30) * 0.1f * sqrtf(difficulty) : 0;
    bird.vx += ((input.isPressed ? 3 * sqrtf(difficulty) : 0.1f) - bird.vx) * 0.2f;
    bird.ticks += bird.vx;
    ...
    if (nextFishTicks < 0) {
        ...
        Vector vel = {-rnd(1, 1.5f) * 0.5f * sqrtf(difficulty), -2.5f * sqrtf(difficulty)};
        ...
        f->type = type;
        f->isAlive = true;
        fishIndex = cgl_wrap(fishIndex + 1, 0, MAX_FISHES);
    ...
    scrX = cgl_wrap(scrX - scr, -10, 210);
    color = BLUE;
    rect(0, 50, 200, 2);
    color = CYAN;

    for (int i = 0; i < 5; i++) {
        rect(cgl_wrap(scrX + (220.0f / 5) * i, -10, 210), 50, 9, 2);
    }
