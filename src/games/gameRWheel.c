...
static void addBonus() {
  TIMES((int)ceilf((float)validSpikeCount / 9), i) {
    ...
    b->radius = rnd(10, 30);
    b->isAlive = true;
    bonusIndex = cgl_wrap(bonusIndex + 1, 0, RWHEEL_MAX_BONUS_COUNT);
  }
}
...
static void update() {
  ...
  if (player.y < 0) {
    float pvy = player.vy;
    player.vy += (input.isPressed ? 1 : 3) * 0.03f * difficulty;
    player.vy *= 0.98f;
    ...
    if (pvy * player.vy <= 0) {
      ...
      b->isSpike = true;
      b->isAlive = true;
      barIndex = cgl_wrap(barIndex + 1, 0, RWHEEL_MAX_BAR_COUNT);
    }
    player.y += player.vy;
    if (player.y > 0) {
  ...
  FOR_EACH(spikes, i) {
    ...
    Vector p;
    addWithAngle(vectorSet(&p, 50, 50), a, wheelRadius * (1 - s->height * 0.1f));
    Vector bp;
  ...
  if (validSpikeCount == 0) {
    spikes[(int)cgl_wrap(
               ...
               RWHEEL_SPIKE_COUNT)]
        .height = 1;
    spikes[(int)cgl_wrap((int)(-angleOfs - M_PI / 4 * 3) /
