...
static void update() {
  ...
  thickness = 2;
  if (nodeState == 1) {
    throwPower += 0.05f * sqrtf(difficulty);
    float a = 0.1f - throwPower * 0.2f;
    Vector p;
    ...
    if (input.isJustReleased || throwPower > 3) {
      ...
      nodeState = 2;
      rotate(vectorSet(&nodes[0].vel, sqrtf(difficulty) * throwPower, 0), a);
    }
  }
  if (nodeState == 2) {
    FOR_EACH(nodes, i) {
      ...
      if (i == 0) {
        ...
        n->vel.y += (n->pos.y < waterY ? 0.05f : 0.01f) * difficulty;
        ...
        if (!line(VEC_XY(n->nextNode->pos), VEC_XY(n->pos))
          ...
          n->vel.y += (n->pos.y < waterY ? 0.005f : 0.001f) * difficulty;
          vectorMul(&n->vel, 0.99);
        }
      }
    }
  ...
  if (nodeState == 3) {
    FOR_EACH(nodes, i) {
      ...
      if (distanceTo(&n->pos, VEC_XY(startPos)) > 1 && n->pos.x > startPos.x) {
        ...
        addWithAngle(&n->pos, a, sqrtf(difficulty) * 2);
      } else {
        vectorSet(&n->pos, VEC_XY(startPos));
        if (i == 0) {
          nodeState = 0;
  ...
  if (nextFishTicks < 0) {
    ...
    float vx = -rnd(1, sqrtf(difficulty)) * (type == 0 ? 0.3f : 0.2f);
    TIMES(c, i) {
      ...
      f->type = type;
      f->isAlive = true;
      fishIndex = cgl_wrap(fishIndex + 1, 0, CAST_N_MAX_FISH_COUNT);
    }
    nextFishTicks = rnd(120, 150) / sqrtf(difficulty);
  }
  FOR_EACH(fishes, i) {
    ...
    if (c.rect[BLACK] || c.character['a']) {
      if (nodeState == 3) {
        ...
        addWithAngle(&f->pos, a, sqrtf(difficulty) * 2);
        f->pos.y -= difficulty * 0.3f;
        if (f->pos.x < startPos.x + 9) {
          ...
          multiplier++;
          waterY =
              clamp(waterY + (f->type == 0 ? -5 : 1) * sqrtf(difficulty), 0, 50);
    ...
    f->vel.x += rnd(0, sqrtf(difficulty)) * RNDPM() * 0.01f;
