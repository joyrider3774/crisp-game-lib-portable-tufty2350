...
static void update() {
  if (!ticks) {
    ...
    f->hasChick = false;
    f->isAlive = true;
    floorIndex = cgl_wrap(floorIndex + 1, 0, IN_TOW_MAX_FLOOR_COUNT);
  ...
  if (bird.isJumping) {
    if (chickCount > 0 && input.isJustPressed) {
      ...
      fc->isAlive = true;
      fallingChickIndex =
          cgl_wrap(fallingChickIndex + 1, 0, IN_TOW_MAX_FALLING_CHICK_COUNT);
    }
    Vector pp;
    vectorSet(&pp, VEC_XY(bird.pos));
    bird.vy += (input.isPressed ? 0.05f : 0.2f) * difficulty;
    bird.pos.y += bird.vy;
    Vector op;
  ...
  if (nextBulletDist < 0) {
    ...
    b->vx = rnd(1, difficulty) * 0.3f;
    b->isAlive = true;
    bulletIndex = cgl_wrap(bulletIndex + 1, 0, IN_TOW_MAX_BULLET_COUNT);
  ...
  for (int i = 0; i < chickCount; i++) {
    Chick* c = &(chicks[i]);
    c->targetIndex = 3 * (i + 1);
    c->index += (c->targetIndex - c->index) * 0.05f;
