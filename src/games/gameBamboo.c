...
static void update() {
    ...
    playerPos.x = cgl_wrap(playerPos.x + vx * difficulty * (1 + avx), -3, 203);
    playerPos.y = 87;
    avx *= 0.9f;
    ...
    if (nextBambooTicks < 0) {
        ...
        b->isAlive = true;
        bambooIndex = cgl_wrap(bambooIndex + 1, 0, MAX_BAMBOO_COUNT);
    ...
    FOR_EACH(bamboos, i) {
        ...
        b->height += b->speed * difficulty * 0.14f;
        float h = b->height / 4;
        float y = 90 - h / 2;
        if (h < 1) {
            y += (1 - h) * 3;
        ...
        for (int j = 0; j < 4; j++) {
            ...
            Collision cl = box(b->pos.x, y, (4 - j) * 2, ceilf(h));
        ...
        if (collision) {
            if (b->height < 5) {
                ...
                vx *= -1;
                particle(b->pos.x, playerPos.y, 9, difficulty * (1 + avx) * 0.5f, vx > 0 ? 0 : M_PI, 0.4f);
                if (b->height <= 25) {
                    play(EXPLOSION);
                    b->isAlive = false;
                    continue;
