...
static void update() {
    ...
    const float wallStopInterval = 120 / sqrtf(difficulty);
    ...
    if (nextWallDist < 0) {
        ...
        for (int attempt = 0; attempt < 99; attempt++) {
            float vx = rnd(5, 10) * RNDPM() * sqrtf(difficulty); // rnds(5, 10)
            if (fabsf(jsModulo(vx * wallStopInterval * 0.5f, w + i)) > 19) {
                wall->vx = vx;
                isValid = true;
                break;
            }
        ...
        wallIndex = cgl_wrap(wallIndex + 1, 0, MAX_WALLS);
        nextWallDist += 11;
    }
