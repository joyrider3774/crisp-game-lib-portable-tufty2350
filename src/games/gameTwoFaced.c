...
static void update() {
    ...
    if (activeTailCount < MAX_TAILS) {
        ...
        t->angle = head.angle;
        t->isAlive = true;
        tailIndex = cgl_wrap(tailIndex + 1, 0, MAX_TAILS);
    ...
    // Update multiplier
    int pp = ceilf(multiplier);
    multiplier -= 0.01f;
    if (multiplier < 1) {
        multiplier = 1;
    }
    if (ceilf(multiplier) < pp) {
        play(COIN);
    }

    color = BLACK;
    char multText[16];
    sprintf(multText, "x%d", (int)ceilf(multiplier));
    text(multText, 3, 9);
}
