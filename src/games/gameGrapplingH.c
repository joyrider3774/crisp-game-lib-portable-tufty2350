...
static void addAnchorPoint(float x, float y) {
  ...
  ap->pos.y = y;
  ap->isAlive = true;
  anchorPointIndex = cgl_wrap(anchorPointIndex + 1, 0, MAX_ANCHOR_POINTS);
}
...
static void update() {
  ...
  float scrollY = 0.1f * difficulty;
  if (player.pos.y < 75) {
    scrollY += (75 - player.pos.y) * 0.1f;
  }
    ...
    if (ap == attachableAnchor) {
      ...
      if (inputIsReleased && input.isPressed) {
        ...
        if (scrolledDistanceYForScore > 0) {
          addScore(ceilf(sqrtf(scrolledDistanceYForScore * scrolledDistanceYForScore) * 0.1f), VEC_XY(ap->pos));
        }
        inputIsJustPressed = false;
      }
    }
  ...
  vectorMul(&player.vel, 1 - fabsf(vectorLength(&player.vel)) * 0.001f);
  player.vel.y += 0.1f;
  ...
  if (player.attachedAnchor != NULL) {
    ...
    float force = dist;
    addWithAngle(&player.vel, angle, force * 0.01f);
    ...
    if (inputIsJustPressed) {
      ...
      addWithAngle(&player.vel, angle, force * 0.2f);
      scrolledDistanceYForScore = 0;
      inputIsReleased = false;
    }
  } else {
