...
static void update() {
  ...
  if (nextSpikeDist < 0) {
    ...
    s->vy = rnd(0, 1) < 0.2f ? rnd(1, difficulty) * RNDPM() * 0.3f : 0;
    s->isAlive = true;
    nextSpikeDist += rnd(9, 49);
    spikeIndex = cgl_wrap(spikeIndex + 1, 0, BALL_TOUR_MAX_SPIKE_COUNT);
  }
  color = BLACK;
  FOR_EACH(spikes, i) {
  ...
  if (nextBallDist < 0) {
    ...
    if (character("c", VEC_XY(p)).isColliding.text['*']) {
      ...
      ballIndex = cgl_wrap(ballIndex + 1, 0, BALL_TOUR_MAX_BALL_COUNT);
    }
  }
  multiplier = clamp(multiplier - 0.02f * difficulty, 1, 999);
  color = BLACK;
  text("x", 3, 9);
