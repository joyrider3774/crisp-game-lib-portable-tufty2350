...
static void update() {
  ...
  if (lc == 0) {
    ...
    vectorSet(&l->vel, 0.5f * difficulty, 0);
    rotate(&l->vel, M_PI / 2);
    l->ticks = ceilf(30.0f / difficulty);
    l->prevLine = NULL;
    l->isActive = false;
    l->isAlive = true;
    lineIndex = cgl_wrap(lineIndex + 1, 0, THUNDER_MAX_LINE_COUNT);
  }
  color = LIGHT_BLUE;
  rect(0, 90, 100, 10);
  activeTicks--;
  FOR_EACH(lines, i) {
    ...
    if (activeTicks > 0) {
      if (l->ticks > 0) {
        ...
        s->vy = -l->to.y * 0.02f;
        s->isAlive = true;
        starIndex = cgl_wrap(starIndex + 1, 0, THUNDER_MAX_STAR_COUNT);
      }
      l->isAlive = false;
      continue;
    }
    if (l->ticks > 0) {
      ...
      for (int j = 0; j < rndi(1, 4); j++) {
        ...
        vectorMul(&nl->vel, 1.0f / vectorLength(&nl->vel));
        rotate(&nl->vel, rnd(-0.7, 0.7));
        vectorMul(&nl->vel, rnd(0.3, 1) * sqrtf(difficulty));
