...
static void addLane(bool isEmpty) {
  nextEmptyLaneCount--;
  float vr = sqrtf(difficulty) + 0.1f;
  float vx = (rnd(0, vr) * rnd(0, vr) * rnd(0, vr) + 1) * 0.3f * RNDPM();
  ...
  l->isAlive = true;
  laneIndex = cgl_wrap(laneIndex + 1, 0, FROOOOG_MAX_LANE_COUNT);
  nextLaneY -= 10;
...
static void updateLanes() {
  FOR_EACH(lanes, i) {
    ...
    if (l->ticks < 0) {
      ...
      c->color = l->color;
      c->isAlive = true;
      carIndex = cgl_wrap(carIndex + 1, 0, FROOOOG_MAX_CAR_COUNT);
...
static void update() {
  ...
  if (frog.state == 1) {
    frog.targetY -= sqrtf(difficulty) * 0.7f;
    color = LIGHT_BLACK;
