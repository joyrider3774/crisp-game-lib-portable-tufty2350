...
static void addBar(float x, float length, float angle, float angleVel, bool isHeld) {
    ...
    b->isHeld = isHeld;
    b->isAlive = true;
    barIndex = cgl_wrap(barIndex + 1, 0, AERIAL_BAR_MAX_COUNT);
}
...
static void update() {
    ...
    ceilingY += (targetCeilingY - ceilingY) * 0.1f;
    color = LIGHT_CYAN;
    rect(0, 0, 100, ceilingY);
    color = LIGHT_BLUE;
    rect(0, 90, 100, 10);

    float scr = difficulty * 0.02f;
    if (player.pos.x > 50) {
        scr += (player.pos.x - 50) * 0.1f;
    }
    player.pos.x -= scr;
    ...
    if (player.bar != NULL) {
        ...
        player.angleVel += b->angleVel * b->length * 0.003f;
        ...
        if (input.isJustPressed) {
            play(SELECT);
            vectorSet(&player.vel, 0, 0);
            addWithAngle(&player.vel, b->angle + M_PI / 2.0f, 
                (b->angleVel * b->length + player.angleVel * 3) * sqrtf(difficulty));
            vectorAdd(&player.vel, 0, -sqrtf(difficulty) * 0.5f);
            player.bar = NULL;
            flyingTicks = 1;
        }
    } else {
        ...
        player.vel.y += (input.isPressed ? 0.01f : 0.1f) * sqrtf(difficulty);
        vectorMul(&player.vel, input.isPressed ? 0.99f : 0.95f);
    ...
    while (nextBarDist < 0) {
        ...
        addBar(110, length, M_PI / 2.0f - rnd(0, M_PI / 4), 
    ...
    FOR_EACH(bars, i) {
        ...
        b->angleVel += cosf(b->angle) * b->length * 0.00005f * sqrtf(difficulty);
        b->angle += b->angleVel;
        ...
        if (!b->isHeld && player.bar == NULL && c.isColliding.rect[CYAN]) {
            play(POWER_UP);
            if (flyingTicks > 0) {
                addScore(ceilf(flyingTicks), VEC_XY(endPos));
            }
            player.bar = b;
            b->isHeld = true;
