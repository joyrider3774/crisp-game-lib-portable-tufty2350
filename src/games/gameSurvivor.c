...
static void update() {
  ...
  FOR_EACH(players, i) {
    ...
    if (p->underFoot != NULL) {
      ...
      if (p->pos.x < 50) {
        p->vel.x += 0.01f * sqrtf(50 - p->pos.x + 1);
      } else {
        p->vel.x -= 0.01f * sqrtf(p->pos.x - 50 + 1);
      }
      if (p->isOnFloor) {
        if (p->pos.x < barrel.pos.x) {
          p->vel.x -=
              (0.1f * sqrtf(barrel.r)) / sqrtf(barrel.pos.x - p->pos.x + 1);
        }
      } else {
        p->vel.y += 0.1f;
        if (p->pos.y > 90) {
