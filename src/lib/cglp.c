...
/// \cond
#define MAX_RECORDED_INPUT_COUNT 2560//5120
/// \endcond
...
static void initGameOver() {
  ...
  if (isReplaying) {
    ...
    for (int i = 0; i < gameCount; i++)
    {
      ...
      if (strcmp(saveData.hiScores[i].title, title) == 0)
      {
          foundIndex = i;
          break;
      }
    ...
    if(index > -1)
    {
      ...
      {
        strncpy(saveData.hiScores[index].title, title, strlen(title));
...
void initHiScores()
{
  ...
  {
    memset(saveData.hiScores[i].title, 0, 20*sizeof(char));
    saveData.hiScores[i].hiScore = 0;
  }
}
