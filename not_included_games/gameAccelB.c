...
#define MAX_PLAYER_MISSILES 50
#define MAX_ENEMIES 50
#define MAX_MISSILES 50
#define MAX_SMOKES 100
...
static void update() {
    ...
    FOR_EACH(smokes, i) {
        ...
        float boxSize = 3 + cosf(s->ticks * 0.03f) * 5;  // Calculate size once
        box(s->pos.x, s->pos.y, boxSize, boxSize);  // Same size for width/height
        
        s->ticks += sqrtf(difficulty);
        if (s->ticks > 60) {
            s->isAlive = false;
        }
    }
    ...
    // Update player
    player.vx += ((input.isPressed ? 2 : 0.2f) * sqrtf(difficulty) - player.vx) * 0.2f;
    ...
    if (activeMissiles == 0) {
        ...
        if (input.isJustPressed) {
            ...
            FOR_EACH(enemies, i) {
                ...
                vectorSet(&v, sqrtf(difficulty) * 2, 0);  // Only X component
                ...
                pm->smokeTicks = 0;
                pm->isAlive = true;
                enemyIndex = cgl_wrap(enemyIndex + 1, 0, MAX_PLAYER_MISSILES);
            }
    ...
    FOR_EACH(playerMissiles, i) {
        ...
        m->pos.x += sqrtf(difficulty) - scr;

        if (m->exTicks > 0) {
            if (m->pos.y > 90) {
                vectorSet(&m->vel, 0, 0);
            }
            m->exTicks += sqrtf(difficulty);
            vectorMul(&m->vel, 0.9f);
            color = RED;
            float boxSize = 3 + cosf(m->exTicks * 0.05f) * 9;
        ...
        float mv = (sqrtf(difficulty) / sqrtf(d + 9)) * 
        ...
        if (m->smokeTicks > 5) {
            ...
            s->isEnemy = false;
            s->isAlive = true;
            smokeIndex = cgl_wrap(smokeIndex + 1, 0, MAX_SMOKES);
            m->smokeTicks -= 5;
        }
        activeMissiles++;
    }
    ...
    if (nextEnemyDist < 0) {
        ...
        e->vx = rnd(0, sqrtf(difficulty)) * 0.5f;
        e->ma = M_PI + rnd(0.2, M_PI / 5) * RNDPM(),
        e->fireTicks = ceilf(rnd(10, 30) / sqrtf(difficulty));
        e->isAlive = true;
        enemyIndex = cgl_wrap(enemyIndex + 1, 0, MAX_ENEMIES);
    ...
    FOR_EACH(enemies, i) {
        ...
        if (e->fireTicks == 0) {
            ...
            m->angle = e->ma;
            m->va = 0;
            m->speed = sqrtf(difficulty);
            m->smokeTicks = 0;
            m->isAlive = true;
            missileIndex = cgl_wrap(missileIndex + 1, 0, MAX_MISSILES);
        }
    ...
    // Update missiles
    float vva = sqrtf(difficulty) * 0.0005f;
    FOR_EACH(missiles, i) {
        ASSIGN_ARRAY_ITEM(missiles, i, Missile, m);
        SKIP_IS_NOT_ALIVE(m);
        ...
        float oy = cgl_wrap(ta - m->angle, -M_PI, M_PI);
        m->va += (oy > 0 ? 1 : -1) * vva;
        m->angle = clamp(cgl_wrap(m->angle + m->va, 0, M_PI * 2),
                        (M_PI / 4) * 3, (M_PI / 4) * 5);
